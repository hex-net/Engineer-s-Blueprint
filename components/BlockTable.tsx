...
export const BlockTable: React.FC<BlockTableProps> = ({ blocks }) => {
  ...
  const requestSort = (key: BlockSortKeys) => {
  ...
  const getSortDirectionFor = (key: BlockSortKeys) => {
  ...
  const renderHeader = (key: BlockSortKeys, title: string, helpText: string, textAlign: 'text-left' | 'text-right' | 'text-center' = 'text-left') => (
    <th 
      scope="col" 
      ...
      <table className="min-w-full divide-y divide-slate-700">
        <thead className="bg-slate-700/50">
          <tr>
            ...
            {renderHeader('name', 'Name', 'User-friendly name of the block.')}
            ...
            <th scope="col" className="px-3 py-3.5 text-center text-xs sm:text-sm font-semibold text-sky-300 whitespace-nowrap">Materials <InfoTooltip text="Components required to build the block. Hover over the icon for details."/></th>
            {renderHeader('pcu', 'PCU', "Performance Cost Unit. Represents the computational load a block places on the game.", 'text-right')}
            {renderHeader('blockPairName', 'Block Pair', "Name used to group variants of the same block (e.g., different armor shapes).")}
            {renderHeader('edgeType', 'Edge Type', "Defines how the block connects visually and structurally (e.g., 'Light' or 'Heavy').")}
            {renderHeader('isAirTight', 'Airtight', "Indicates if the block can form an airtight seal for pressurized environments.", 'text-center')}
            {renderHeader('dlc', 'DLC', 'Downloadable Content pack this block belongs to, if any.')}
          </tr>
        </thead>
