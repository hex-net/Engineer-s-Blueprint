...
export const MaterialsIcon: React.FC<{ components?: BlockComponent[] }> = ({ components }) => {
  if (!components || components.length === 0) {
    return <span className="text-[var(--color-text-muted)]">N/A</span>;
  }
  const title = components.map(c => `${c.subtype}: ${c.count}`).join(', ');
  return (
    <span title={title} aria-label={`Materials: ${title}`}>
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5 text-[var(--color-text-secondary)] hover:text-[var(--color-text-accent)] transition-colors">
...
export const SortIndicator: React.FC<SortIndicatorProps> = ({ direction }) => {
  ...
  return (
    <span className="ml-1 inline-block w-3 h-3 text-[var(--color-text-accent)] opacity-70">
      {direction === 'ascending' ? 
