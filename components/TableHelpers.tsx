...
import { BlockComponent } from '../types'; // Assuming ArmorBlockComponent has same structure
...
export const formatNumber = (num: number | string | undefined): string => {
  ...
  if (typeof num === 'string' && (num.includes('²') || num.includes('³') || num === '-?-')) {
    return num; // Return as is if it's a special string like "0²" or "-?-"
  }
  const numericValue = typeof num === 'string' ? parseFloat(num.replace(/,/g, '')) : num;
  if (isNaN(numericValue)) return String(num); // Fallback for other non-numeric strings
