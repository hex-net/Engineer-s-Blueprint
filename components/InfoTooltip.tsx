...
const TooltipPopup: React.FC<TooltipPopupProps> = ({ text, id, iconRect }) => {
  ...
  return (
    <span
      ...
      id={id}
      role="tooltip"
      className="fixed z-[9999] max-w-xs w-auto p-3 text-xs text-slate-100 bg-slate-800 border border-slate-600 rounded-md shadow-xl whitespace-normal pointer-events-none transition-opacity duration-150"
      style={{
        opacity: opacity,
...
export const InfoTooltip: React.FC<InfoTooltipProps> = ({ text, className }) => {
  ...
  return (
    ...
    >
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 text-sky-400 hover:text-sky-300">
